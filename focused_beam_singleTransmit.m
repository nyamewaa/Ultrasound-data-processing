function [b, x, z] = focused_beam_singleTransmit(rf, acq_params, bf_params, focus_cm)
%This function takes in pulse echo data as input and converts it
%to a focus_cmed beamformed image with pre-defined element and transmit
%pitches, sampling frequency, and focus_cm
    ...
    for i = 1:num_tx % different lateral positions
        el_min_idx = ceil(((tx_pos(21)-bf_params.app_size/2)-left_side_transducer)/bf_params.pitch);
        el_max_idx = floor(((tx_pos(21)+bf_params.app_size/2)-left_side_transducer)/bf_params.pitch);
        for j = el_min_idx:el_max_idx
            dx = abs(tx_pos(21)-rx_pos(j));
